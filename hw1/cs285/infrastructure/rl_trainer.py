...
class RL_Trainer(object):
    ...
    def perform_logging_expert_eval(self, expert_policy):

        # collect eval trajectories, for logging
        print("\nCollecting data for expert eval...")
        eval_paths, eval_envsteps_this_batch = utils.sample_trajectories(self.env, expert_policy,
                                                                         self.params['eval_batch_size'],
                                                                         self.params['ep_len'])

        # save eval rollouts as videos in tensorboard event file
        if self.log_video:
            print('\nCollecting video rollouts expert eval')
            eval_video_paths = utils.sample_n_trajectories(self.env, expert_policy, MAX_NVIDEO, MAX_VIDEO_LEN, True)

            # save train/eval videos
            print('\nSaving expert eval rollouts as videos...')
            self.logger.log_paths_as_videos(eval_video_paths, 0, fps=self.fps, max_videos_to_save=MAX_NVIDEO,
                                            video_title='expert_eval_rollouts')
        ...
        if self.log_metrics:
            ...
            # decide what to log
            logs = OrderedDict()
            logs["Expert_Eval_AverageReturn"] = np.mean(eval_returns)
            logs["Expert_Eval_StdReturn"] = np.std(eval_returns)
            logs["Expert_Eval_MaxReturn"] = np.max(eval_returns)
            logs["Expert_Eval_MinReturn"] = np.min(eval_returns)
            logs["Expert_Eval_AverageEpLen"] = np.mean(eval_ep_lens)

            # perform the logging
            for key, value in logs.items():
                print('{} : {}'.format(key, value))
                self.logger.log_scalar(value, key, 0)
            print('Done logging...\n\n')
